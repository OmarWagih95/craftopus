...
function ProductInfo(params) {
  ...
  return (
    ...
    <div className='text-white items-center justify-items-center justify-center'>
      ...
      <h2 className='text-gray-400 text-[16px]'>{params.product.attributes?.category}</h2>
      <h2 className='mt-5 font-light'>{params.product.attributes?.description[0].children[0].text}</h2>
      <h2 className='text-white text-[26px] font-bold'>{params.product.attributes?.price} <span className='text-purple-700'>$</span>
</h2>
{/* <div className='grid  justify-items-center'> */}

<button className='flex gap-3 mt-5 rounded-lg items-center bg-purple-700 px-3 py-3 hover:bg-purple-500'><FaShoppingCart /> add to cart</button>
{/* </div> */}
    </div>
    :
    <SkeletonProductInfo/>
